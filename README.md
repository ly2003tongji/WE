...
1. 使用 `prompts/H20_AGENT_PROMPT.md` 启动新会话。
2. 先审计系统、代码和数据体积，不直接下载全量数据。
3. 目标顺序：少量场景 smoke test → 288 rare-case baseline → NR/R 对比 → rare-log LoRA。
4. 将环境、命令、失败原因和结果写入 `HANDOFF.md` 或 `reports/`。
