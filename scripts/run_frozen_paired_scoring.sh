...
OUT_DIR="${FROZEN_OUT_DIR:-/mnt/cpfs/prediction/lyyy/myself/WE/data/frozen_paired/smoke1_cutoff3}"
...
mkdir -p "${OUT_DIR}"
echo "[1/3] build_frozen_traffic_futures"
"${PY}" "${SCRIPT_DIR}/build_frozen_traffic_futures.py" \
  --scene-pkl "${SCENE_PKL}" \
  --vocab "${VOCAB}" \
  --out-dir "${OUT_DIR}"
