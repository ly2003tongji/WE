...
**直接回答核心问题**：cold 中的「近静止振荡 + 造成 32% NOC 翻转」**不能**在「从完整场景正常跑到 cutoff 的官方 IDM」（未做 cutoff 物理恢复）中复现为同一模式；warm 表现为另一种病理（完全停滞于起点）。因此在**门控 C（不可比奖励）**下，本阶段**无法**直接判断 32% 翻转是伪影还是真实行为——只能确认 cold-start 的振荡模式本身与"未恢复物理的连续 warm 跑法"不一致。

> **阶段 1.9 更正（见 `reports/IDM_RESTORE_PHYSICS_VALIDATION.md`）**：cold-start振荡模式确认属于截断重初始化伪影相关的工程观察；由于warm-start在cutoff状态不配对（门控C），当时无法直接确定32% NOC/TTC翻转的真实效应量；阶段1.9通过restore-physics严格配对（门控A）重新计算后，确认该翻转在严格配对下依然完全存在且可归因到同一车辆，应视为官方IDM在此场景下的真实行为而非冷启动伪影。

## 5. Warm-start 奖励评分

**跳过**（cutoff 门控 C）。无 warm NOC/TTC flip 数字可报。

## 6. 判定（阶段 1.8 时点；已被阶段 1.9 更正，见上）

| 标准 | 阶段 1.8 结论 | 阶段 1.9 更正后 |
|---|---|---|
| 分类 | `cold_start_artifact`（基于门控 C 下的推测） | **`B_real_disagreement_under_strict_pairing`**（门控 A 下确认） |
| 阶段 1.5–1.7 | 曾仅保留为「发现冷启动伪影」的工程记录 | 32% 翻转**已在严格配对下复现**，应视为该场景下官方 IDM 真实行为 |
| 32% NOC / 28% TTC 翻转 | 曾建议不得用于效应量 | **已通过 restore-physics 严格配对确认**；仍只是 1-scene 工程证据，不构成研究假设成立证据 |
| 扩样 | 曾建议禁止 | 技术条件已具备（restore-physics 协议、门控 A、归因清晰），**仍需用户明确批准**后才可扩样 |

补充：warm（未恢复物理）上该车完全停滞，与 restore-physics（严格恢复物理但保留导航历史）上该车近静止振荡，是**两种不同机制**——前者是"物理漂移未纠正"的伪影，后者是可信的官方 IDM 行为（详见阶段 1.9 报告）。
